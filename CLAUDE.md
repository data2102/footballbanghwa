...
- 함수가 응답마다 `응답: ...ms, 출력 N토큰` 을 남긴다. **한도에 다시 닿으면 이 숫자부터 본다.**
  출력 토큰이 그대로 시간이다.
- **짧은 경로만 `effort: high` 다.** 긴 경로까지 올렸더니 판 한 장에 한참을 기다렸다 —
  짧은 쪽은 출력이 100~400토큰이라 늘어나는 게 생각하는 시간뿐이지만, 긴 쪽은 출력 자체가
  길어서 두 배로 든다.
- **AI 서버는 탭을 여는 순간 깨운다.** 사진 고르는 화면에 들어가서야 깨우면 잠들어 있던
  30~60초를 사람이 그대로 기다린다. 참석·출전 탭이 열릴 때 `wakeAiServer()` 를 부른다.
- **`effort` 를 낮춰 둔 건 그 벽 때문이었다.** Render 로 옮겨 벽이 없어졌으니 짧은 경로도
  `high`(기본값)로 읽는다 — 사진 여섯 장에서 아흔 명을 빠짐없이 골라내는 일에 제일 낮은
  단을 쓸 이유가 없다. 출력은 여전히 400토큰이라 늘어나는 건 생각하는 시간뿐이다.
- **`max_tokens` 에는 생각한 양도 든다.** 답이 짧다고 좁게 잡으면 생각하다 한도에 닿아
  JSON 이 잘리고, 그러면 읽은 게 통째로 날아간다.

### 조각으로 잘라 보낸 사진은 "이어서" 읽게 해야 한다

긴 캡처는 여러 장으로 잘려 나간다. 그런데 카톡 항목별 탭은 **말머리("불참 : 35명") 아래로
이름이 이어지는** 구조라, 조각 하나가 말머리 없이 이름부터 시작하는 일이 생긴다.
프롬프트에 "앞 장의 말머리를 이어 쓴다"가 없으면 모델은 그 이름들을 어느 칸인지 몰라
**조용히 버린다.** 불참이 화면 아래쪽 긴 묶음이라 하필 불참만 골라 빠진다.

빠진 사람은 앱에서 **미정**이 되어 아무 표시 없이 남고, 총무는 그 사람이 화면에 없었는지
우리가 못 읽었는지 알 수가 없다. 그래서 검토 화면이 **읽은 수와 명단 수를 나란히** 보여 준다 —
카톡 화면의 "N명" 머리글과 맞춰 보면 저장 전에 잡힌다.

**근본은 칸을 사람이 골라 주는 것이다.** 참석·불참·미투표를 각각 따로 찍어 올리고 사진마다
칸을 고르면(`statusHint`) **추론할 일 자체가 없어진다** — 모델이 읽을 것은 이름뿐이고,
말머리도 조각 이어 붙이기도 필요 없다.

- 사람이 고른 칸이 모델보다 세다. 함수가 응답의 어느 배열에 담겼든 **전부 그 칸으로 돌린다**
  (`expandRoster(..., fixed)`). 못 찾은 이름도 같은 칸에 선다 — 화면을 보고 고른 사람이 맞다.
- 사진 한 장이 요청 하나라, 장마다 다른 칸을 붙여 **한 번에 세 장을 올릴 수 있다.**
  한 장이 조각 여럿으로 갈려도 그 장의 칸은 조각마다 따라간다.
- 안 고르면 예전 그대로다. 한 화면에 다 들어 있는 캡처는 통째로 올려도 된다.
...
**출전 탭에는 전술판도 포메이션 고르는 줄도 없다.** 라인업은 이미 운동장 화이트보드에서
짜고 왔다 — 앱이 하는 일은 그 사진을 올려 "누가 몇 쿼터를 뛰었나"를 세는 것 하나다.
판 배치를 다시 보고 싶으면 **사진을 눌러 크게 본다.** 원본이 제일 정확하고, 앱이 다시
그린 판은 읽다가 틀린 것까지 그대로 옮긴다.

좌표는 여전히 필요하다 — 단톡방에 나가는 공유 이미지가 판을 그린다(`lineupShare.ts`).
그래서 `LineupSlot` 의 x·y 는 남고, 화면에만 안 그린다.

**사진도 쿼터마다 한 장이다.** 화이트보드 한 장에 두 팀이 다 그려져 있어서 A팀 사진과
B팀 사진을 따로 받으면 같은 사진을 두 번 올리게 된다. 새로 올리는 사진은 A팀 줄에
붙이고(`setLineupPhoto(..., 'A', ...)`), 화면은 A·B 중 있는 쪽을 보여 준다 —
예전에 B팀 쪽에 올려 둔 사진이 안 보이면 없어진 줄 안다.

**포메이션은 팀마다 따로 고르고, 지금 쓰는 인원 규격을 줄 맨 앞으로 당긴다.**
8인제로 잡힌 날에 11인 포메이션이 앞을 다 차지하면 지금 켜진 칩이 화면 밖에 있다.

**화이트보드는 사람이 옮겨 적는 게 아니라 읽는다.** 판에 **이름을 쓴 자석**을 붙이고
네이버 라인업처럼 줄로 세운다. 그래서 포메이션을 고르고 이름을 하나씩 넣을 일이 없다 —
사진 한 장을 `lineup-photo` 가 읽어 두 팀을 채운다.

- **자리 수는 읽은 그대로 쓴다**(`formationFromGroups`). 익숙한 4-3-3 으로 끌어다 붙이면
  수비가 셋인 날에 없는 자리가 하나 생기고, 다섯인 날에는 한 명이 갈 데가 없어진다.
  카탈로그에 같은 줄 수가 있으면 그걸 쓰고(4-2-3-1 처럼 줄이 갈리는 모양이 살아난다),
  없으면 그 수대로 새로 그린다.
- **판 사진은 자르지 않는다**(`pickPhoto(..., { whole: true })`). 자르기는 긴 카톡 캡처의
  작은 글씨를 살리려고 있는 것인데, 판은 **배치가 곧 내용**이다 — 위 덩어리가 A팀, 아래가
  B팀이고 자기 골문에서 먼 줄일수록 공격이다. 가로로 자르면 아랫조각은 하프라인도 골문도
  없는 띠가 되어 어느 팀 어느 줄인지 알 길이 없다. 자석 글씨는 크고, 줄여도 긴 변이
  1568 안에 들어와서 잘라 봐야 얻을 것도 없다.
- **골키퍼 자석이 없는 판이 흔하다.** 골문이 비어 있으면 GK 를 지어내지 않고 골문에서 제일
  가까운 줄을 DF 로 읽는다. 10명 4-1-2-3 이면 자리도 열 개다.
- **판도 짧은 출력 경로로 읽는다**(`BOARD_SCHEMA`). 긴 쪽은 한 사람에 약 115토큰이라 두 팀
  스무 명이면 출력만 2,300토큰이고, 그게 그대로 운동장에서 기다리는 시간이 된다. 번호와
  줄만 받으면 150토큰이면 끝난다. 조건은 `hint === 'lineup'` + 사진 있음 + 글 없음이다.
- **줄(line)을 그대로 받는다.** 그룹(DF/MF/FW)만 받으면 4-1-2-3 처럼 줄이 넷인 판이
  4-3-3 이 되어 가운데 두 줄이 합쳐진다. `LineupItem.line` 이 그 줄 번호이고
  `formationFromLines` 가 그대로 그린다. 줄을 모르면 예전처럼 그룹으로 접는다.
- **판을 옮기는 계산은 화면 밖에 둔다**(`features/lineup/board.ts`). 한 명이 갈 데가 없어
  사라져도 판은 멀쩡해 보이고 사람만 없다 — 눈으로 못 잡으니 따로 떼어 두고 검사한다.

**사진은 그 쿼터에 누가 뛰었는지의 원본이다.** 그래서 올린 뒤에도 계속 보여야 한다 —
읽어 낸 이름이 맞는지 대조할 데가 거기뿐이다. 자석 글씨는 카드 안 미리보기로는 안 읽히니
눌러서 화면 가득 띄우고(`PhotoViewer`), 확대는 맞춤·크게 두 단만 둔다. 손가락으로 벌리는
확대는 웹과 네이티브가 달라서 한쪽에서 "안 커지는" 사진이 된다.

- **비공개 버킷이라 읽을 때 서명 URL 을 새로 만들어야 한다**(`attachLineupUrls`). 이게 없어서
  **올린 직후에만 보이고 다시 들어가면 사라졌다** — 올릴 때 만든 URL 은 그 자리의 메모리에만
  있고, 다시 읽으면 `photoPath` 만 남는다. 회원 사진·영수증과 같은 방식이다.

**용병은 회원으로 만들지 않는다.** 가끔 명단에 없는 사람이 온다. 회원으로 만들면
명단·참석률·회비 독촉에 계속 남아서 안 오는 사람이 매주 미납자로 뜬다. 그래서
`LineupSlot.guestName` 에 판에 적힌 이름만 담고, **이름 왼쪽에 N** 을 붙인다.

- `slots` 는 jsonb 라 칸을 더해도 마이그레이션이 필요 없다. 대신 **예전에 저장된 줄에는
  이 칸이 없으므로** 읽을 때 `guestName ?? null` 로 맞춘다.
- 판·목록·단톡방 공유 글·공유 이미지 **네 군데 모두**에 N 이 나와야 한다. 한 곳이라도
  빠뜨리면 그 사람은 거기서만 사라진다.

**출전 명단은 세 명씩 한 줄이다.** 마흔 명을 한 줄씩 늘어놓으면 화면이 너무 길어져서
아래쪽 사람은 스크롤해야 보인다. 세 칸으로 놓으면 한 화면에 열두 명이 들어와 "다음에 넣을
사람"이 위에서 다 보인다. 칸 하나에 이름·뛴 횟수·연령대·포지션·지금 팀.

- **덜 뛴 순이 먼저고, 같은 횟수 안에서는 나이 많은 분부터**다(회원 탭의 연령대 순과 같은 차례).
- **대기와 출전을 한 목록에 둔다.** 따로 두면 덜 뛴 사람이 대기에 있는지가 위쪽에서 안 보인다.
- 칸을 누르면 **대기 → A팀 → B팀 → 대기**로 돈다. 전술판이 없으니 누르는 것 하나로 팀을 옮긴다.
- 자리가 다 차 있으면 **한 자리를 더 만든다.** 자리 수를 미리 정해 둘 이유가 없다 —
  판에서 읽은 대로 시작해서 사람이 더하는 만큼 늘어난다.
