 ...
 */
/**
 * 요청 하나에 실을 수 있는 조각 수. `photo.ts` 의 MAX_TILES 와 같거나 커야 한다.
 *
 * 전에는 4였다. 그런데 긴 캡처는 조각이 대여섯 개라, 한 사진이 두 요청으로 갈렸다.
 * 두 번째 요청은 "40대 참석 : 13명" 같은 말머리가 없는 조각들만 받아서 무슨 칸인지
 * 알 수가 없다 — 그래서 불참 서른다섯 명 중 스무 명이 빠지거나 미투표로 들어갔다.
 * 실행 시간 한도가 없는 서버로 옮겼으니 한 사진은 한 요청으로 통째로 보낸다.
 */
const MAX_SLICES_PER_CALL = 6;
