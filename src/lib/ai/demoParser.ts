...
/*
 * 실제 화면은 "40대 참석 : 13명" 처럼 연령대와 인원이 붙는다.
 * 앞뒤 군더더기를 허용하되, "참석"이 들어 있으면 연령대와 무관하게 참석이다.
 */
const COUNT_TAIL = /[\s:：]*\(?\d+\)?\s*명?\s*$/;
const HEAD_ABSENT = new RegExp(`^\\s*(불참|불가|참석\\s?불가|안\\s?감)${COUNT_TAIL.source}`);
const HEAD_ATTEND = new RegExp(
  `^\\s*(?:[0-9]{2}(?:~[0-9]{2})?대\\s*)?(참석|참여|가능)${COUNT_TAIL.source}`,
);
