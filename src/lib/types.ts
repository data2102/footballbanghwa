...
export type LineupSlot = {
  ...
  memberId: string | null;
  /**
   * 회원이 아닌 사람이 이 자리에 섰을 때 화이트보드에 적혀 있던 이름.
   *
   * 가끔 용병이 온다. 그 사람을 회원으로 만들어 버리면 명단·참석률·회비 독촉에 계속
   * 남아서, 안 오는 사람이 매주 미납자로 뜬다. 그래서 **회원으로 만들지 않고 이름만
   * 이 자리에 붙인다.** 화면에서는 이름 왼쪽에 N 을 달아 회원이 아님을 표시한다.
   *
   * memberId 가 있으면 이 칸은 무시한다 — 둘 다 채워지는 일은 없다.
   */
